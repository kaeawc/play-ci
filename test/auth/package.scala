...
package object auth extends Specification {
  ...
  def createUser(email:String,password:String,retyped:String) = {

    val header = FakeRequest(POST, "/signup")

    val data = Json.obj("email" -> email, "password" -> password, "retyped" -> retyped)
    ...
    User.getByEmail(email) map {
      case Some(user:User) => {
        ...
        user.password mustEqual crypto.PBKDF2.hash(password,salt)
      }
